...
async function refreshRelayResponseCache(): Promise<RelayApiRow[]> {
...
async function getRelayApiRowsCached(): Promise<RelayApiRow[]> {
  const now = Date.now();
  ...
  if (relayResponseCache) {
    void refreshRelayResponseCache().catch((error) => {
      console.warn(
  ...
  return refreshRelayResponseCache();
}
  ...
  try {
    ...
    if (relayResponseCache) {
      return NextResponse.json(
        {
          success: true,
          result: relayResponseCache.result,
        },
        { headers: cacheHeaders() },
      );
    }
