...
export async function GET(request: Request) {
  ...
  const paginationCountTotal =
    searchParams.get("paginationCountTotal") ?? undefined;
  ...
  const params = {
    ...
    paginationCountTotal: paginationCountTotal
      ...
      : undefined,
    paginationReverse: paginationReverse
  ...
  const responses = await Promise.all(
    Array.from(
      { length: Math.ceil(total / params.paginationLimit) },
      async (_, i) => {
        return api.poktNetwork.poktroll.supplier.supplier({
          ...params,
          paginationOffset: i * params.paginationLimit,
        });
      }
    )
  );
